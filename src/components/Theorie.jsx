...
import UploadTheorieDialog from "./UploadTheorieDialog";

const Theorie = async ({ params }) => {
  const supabase = createClient();
  const [id] = params.hoofdstuk.split("-");
  const theoryList = await selectTheoryByChapterId(supabase, id); // Ophalen van meerdere theorieën
  ...
  return (
    <div className="w-full flex flex-col space-y-4">
      {theoryList && theoryList.length > 0 ? (
        theoryList.map((theory, index) => (
          <div
            key={theory.id}
            className={`flex flex-col md:flex-row justify-between items-center pb-4 ${
              index !== theoryList.length - 1 ? "border-b border-gray-300" : ""
            }`}
          >
      ...
      {isAdmin && (
        <div className="mb-4">
          <UploadTheorieDialog params={params} id={id} />
          {/* <Dialog>
